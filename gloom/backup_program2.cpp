...
void createCubeFrameBuffer(int size, unsigned int* framebuffer, unsigned int* colorTexture){
  ...
  glFramebufferTextureARB(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, *colorTexture, 0);
...
void runProgram(GLFWwindow* window)
{
    ...
    createCubeFrameBuffer(1024, &cube_framebuffer, &cube_texture);
    ...
    unsigned int view_cube_uniform = glGetUniformLocation(toCubeShader.get(), "view");
    unsigned int projection_cube_uniform = glGetUniformLocation(toCubeShader.get(), "projection");
    unsigned int views_cube_uniform = glGetUniformLocation(toCubeShader.get(), "views");
    ...
    while (!glfwWindowShouldClose(window))
    {
      ...
      glUseProgram(toCubeShader.get());
      glUniformMatrix4fv(view_cube_uniform, 1, GL_FALSE, glm::value_ptr(view));
      glUniformMatrix4fv(projection_cube_uniform, 1, GL_FALSE, glm::value_ptr(projection)); 
      glBindFramebuffer(GL_FRAMEBUFFER_EXT, cube_framebuffer);
      glDrawBuffer(GL_COLOR_ATTACHMENT0_EXT);
